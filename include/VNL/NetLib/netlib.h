...
#define sqrtf(f) ((float)sqrt((double)(f)))
  ...
  /*: Factors a symmetric positive definite matrix and estimates the condition of the matrix (single precision version) */
  void spoco_(float* a, int* lda, int* n, float* rcond, float* z, int* info);

  /*: Computes determinant and inverse of a certain symmetric positive definite matrix using spoco_, sposa_ or sqrdc_ output (single precision version) */
  void spodi_(float* a, const int* lda, const int* n, float* det, const int* job);

  /*: Factors a single precision symmetric positive definite matrix (single precision version) */
  void spofa_(float* a, int* lda, int* n, int* info);

  /*: Solves the symmetric positive definite system a * x = b spoco or spofa output (single precision version) */
  void sposl_(const float* a, const int* lda, const int* n, float* b);
