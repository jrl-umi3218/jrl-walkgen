// This is vxl/VNL/real_polynomial.h
...
*/
  RealPolynomial(VNL::Vector<double> const & a): coeffs_(a) {}

/** Initialize polynomial from C vector.
* The parameter len is the number
* of coefficients, one greater than the degree.
*/
  RealPolynomial(double const * a, int len): coeffs_(a, len) {}

/** Initialize polynomial from double.
* Useful when adding or multiplying a polynomial and a number.
*/
  RealPolynomial(double a): coeffs_(1) { coeffs_[0] = a; }

/** Initialize polynomial of a given degree.
*/
  RealPolynomial(int d): coeffs_(d+1) {}

  bool operator==(RealPolynomial const& p) { return p.Coefficients() == coeffs_; }
...
*/
  double EvaluateIntegral(double x1, double x2) const;
...
*/
  RealPolynomial Primitive() const;
...
*/
  ...
  void SetCoefficients(VNL::Vector<double> const& coeffs) {coeffs_ = coeffs;}
...
*/
RealPolynomial operator+(const RealPolynomial& f1, const RealPolynomial& f2);
...
* \f$\frac1{\sqrt{|x_2-x_1|}}\,\sqrt{\int_{x_1}^{x_2}\left(f_1(x)-f_2(x)\right)^2\,dx}\f$
*/
double RMSDifference(const RealPolynomial& f1, const RealPolynomial& f2,
                          double x1, double x2);

}; // End namespace VNL

#endif // RealPolynomial_h_
