...
int main(void)
{
  ...
  RelativeFootPosition TabFoots[7] = {
    { 0.0, -sy/2, 0.0, 0.0, 0.0, 1},
    {  sx,  sy, 0.0, 0.0, 0.0, 1},
    {  sx, -sy, 0.0, 0.0, 0.0, 1},
    {  sx,  sy, 0.0, 0.0, 0.0, 1},
    {  sx, -sy, 0.0, 0.0, 0.0, 1},
    {  sx,  sy, 0.0, 0.0, 0.0, 1},
    { 0.0, -sy, 0.0, 0.0, 0.0, 1}
    
  };
#endif
  RelativeFootPosition TabFoots2[12] = {
    //  RelativeFootPosition RelativeFootPositions = {
    { 0.0, -sy/2, 0.0, Tsingl, Tdble, 1, 0.0},
    {  sx,  sy, 0.0,  Tsingl, Tdble, 1, -0.005},
    {  sx, -sy, 0.0, Tsingl, Tdble, 1, 0.005},
    {  sx,  sy, 0.0, Tsingl, Tdble, 1, -0.005},
    {  sx, -sy, 0.0, Tsingl, Tdble, 1, 0.01},
    {  sx,  sy, 0.0, Tsingl, Tdble, 1, -0.005},
    {  sx, -sy, 0.0, Tsingl, Tdble, 1, 0.015},
    {  sx,  sy, 0.0, Tsingl, Tdble, 1, -0.005},
    {  sx, -sy, 0.0, Tsingl, Tdble, 1, 0.02},
    {  sx,  sy, 0.0, Tsingl, Tdble, 1, -0.005},
    {  sx, -sy, 0.0, Tsingl, Tdble, 1, 0.0},
  ...
  ofstream arof, alof;
  alof.open("LeftLegJointAngles.csv",ofstream::out);
  arof.open("RightLegJointAngles.csv",ofstream::out);
  ...
  */
  if (walkmode == 1)
    ...
    {
      aStOvPl->CalculateFootHolds(RelativeFootPositions);
    }
  //#endif
  ...
  FootAbsolutePosition InitLFPos, InitRFPos;
  ...
  ofstream aof_COMPos;
  aof_COMPos.open("COMPositions_1.txt", ofstream::out);
  ...
  aof_Finaldqleg.open("Finaldqleg.txt", ofstream::out);
  ...
  if(1)
	...
	BodyAnglesIni(12+i,0)= UpperBodyAnglesBuffer(0,i);
      ...
      VNL::Vector<double> lCOMStartPosition(3,1,0.0);
      aZMPpcwmbz->Setup(ZMPPositions,
			aCOMBuffer,
			LeftFootPositions,
			RightFootPositions,BodyAnglesIni);
      ...
      for(int i=12;i<a2DMB->NbOfLinks();i++)
	{
      ...
      deque<COMPosition> aExtraCOMBuffer;
      deque<ZMPPosition> aExtraZMPBuffer;
      deque<ZMPPosition> aExtraZMPRefBuffer;
      deque<FootAbsolutePosition> aExtraRightFootBuffer, aExtraLeftFootBuffer;
      ...
      for(unsigned int i=NL;i<ZMPPositions.size()-NL;i++)
	{
	  ...
	  VNL::Matrix<double> UpperBodyAngles(28,1);
	  ...
	  WaistPosition.z = aCOMPosition.hip-0.705;
      
	  WaistVelocity.x = aCOMPosition.x[1];
	  WaistVelocity.y = aCOMPosition.y[1];
	  WaistVelocity.z = aCOMPosition.z[1];
	  ...
	  */
	  ...
	  //if(0)
	  {
	    if(aof_Finaldqleg.is_open())
	      {
		aof_Finaldqleg << lql(3,0) <<" ";
		aof_Finaldqleg << lqr(3,0) <<" ";
		for(unsigned int j=0;j<6;j++)
		  aof_Finaldqleg << dql(j,0) <<" ";
		

		for(unsigned int j=0;j<6;j++)
		  aof_Finaldqleg << dqr(j,0) <<" ";
		aof_Finaldqleg << endl;
	    ...
	    if (aof_ARMANGLES.is_open())
	      {
		for(unsigned int j=0;j<7;j++)
		  aof_ARMANGLES << lqArml(j,0) << " ";
		

		for(unsigned int j=0;j<7;j++)
		  aof_ARMANGLES << lqArmr(j,0) <<" ";
 		
		aof_ARMANGLES << LeftFootPositions[i].stepType <<" ";
		...
		aof_COMPos << ZMPPositions[i-NL].time << " " 
			   << ZMPPositions[i-NL].px << " " 
			   << ZMPmultibody[0] << " " 
			   << aCOMPosition.x[0] << " " << aCOMPosition.y[0] << " "
			   << aCOMPosition.z[0] << " " << aCOMPosition.x[1] << " "
			   << aCOMPosition.y[1] << " " << aCOMPosition.z[1] << " "
			   << LeftFootPositions[i-NL].x << " " << RightFootPositions[i-NL].x << " "
			   << LeftFootPositions[i-NL].y << " " << RightFootPositions[i-NL].y << " "
			   << LeftFootPositions[i-NL].z << " " << RightFootPositions[i-NL].z << endl;
	      }
