...
ObstaclePar ObstaclePars;
  ...
  vector<int>  Path;
   ...
   Path = aDMB->trouverCheminEntre(2,12);


for(unsigned int i=0;i<Path.size();i++)
  {
 	cout << "body number:  " << Path[i] << endl;
  }
...
*/
  ...
  aStOvPl->PolyPlanner(aCOMBuffer,LeftFootPositions,RightFootPositions);
  ...
  ofstream aof_COMPos;
  aof_COMPos.open("COMPositions_1.csv", ofstream::out);
	  ...
	  if (aof_COMPos.is_open())
	    {
	      aof_COMPos << ZMPPositions[i-NL].time << " " 
			...
			<< ZMPmultibody[0] << " " 
			 ...
			 << aCOMPosition.y[1] << " " << aCOMPosition.z[1] << " 0.68"<< endl;
	    }
