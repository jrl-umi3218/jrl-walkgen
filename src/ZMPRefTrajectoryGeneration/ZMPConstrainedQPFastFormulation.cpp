...
int ZMPConstrainedQPFastFormulation::ValidationConstraints(double * & DPx,double * &DPu,
							   int NbOfConstraints,
...
int ZMPConstrainedQPFastFormulation::validateConstraints(double * & DS,double * &DU,
									  ...
									  unsigned int N)
{
  ...
  double *DS=0,*DU=0;
  unsigned int NbOfConstraints=4*N; // Nb of constraints to be taken into account
  // for each iteration
 ...
 for(double StartingTime=0.0;
      ...
      if (CriteriaToMaximize==1)
	{
	  ...
	  MAL_VECTOR_RESIZE(OptD,2*(N+Support->StepNumber));
	...
	iwar[0]=1;
      ...
      m = 0;//NbOfConstraints;
      me= 0;
      mmax = m+1;//NbOfConstraints+1;
      n = 2*(N+Support->StepNumber);
      nmax = n; // Size of the matrix to compute the cost function.
      mnn = m+n+n;

      lwar = 3*nmax*nmax/2+ 10*nmax  + 2*mmax + 2000000;

      ODEBUG("m: " << m);
      dumpProblem(m_Q, D, DU, m, DS,XL,XU,StartingTime);
							   ...
							   FootAbsolutePosition & InitRightFootAbsolutePosition)
{
  ...
  printf("Proceeded in GetZMPDiscretization \n");
