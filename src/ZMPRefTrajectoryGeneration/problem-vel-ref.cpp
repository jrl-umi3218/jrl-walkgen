...
void Problem_s::initializeProblem()
{

  memset(DU,0,(8*m_QP_N+1)*2*(m_QP_N+m_stepNumber)*sizeof(double));
}

void Problem_s::dumpMatrix(std::ostream & aos,
			   int type)
{
...
void Problem_s::dumpVector(std::ostream & aos,
			   int type)
{
...
void Problem_s::dumpVector(const char * filename,
			   int type)
{
  std::ofstream aof;
...
void Problem_s::dumpMatrix(const char * filename,
			   int type)
{
  std::ofstream aof;
  aof.open(filename,std::ofstream::out);
...
void Problem_s::dumpProblem(std::ostream &aos)
{
...
void Problem_s::dumpProblem(const char * filename)
{
  std::ofstream aof;
  aof.open(filename,std::ofstream::out);
  dumpProblem(aof);
