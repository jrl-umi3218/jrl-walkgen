...
ZMPVisualServoingQP::~ZMPVisualServoingQP()
{
...
void ZMPVisualServoingQP::setCoMPerturbationForce(double x,double y)
{
  MAL_VECTOR_RESIZE(m_PerturbationAcceleration,6);
...
void ZMPVisualServoingQP::interpolateFeet(deque<FootAbsolutePosition> &,
...
int ZMPVisualServoingQP::InitializeMatrixPbConstants()
{
...
int ZMPVisualServoingQP::BuildingConstantPartOfConstraintMatrices()
{
  if (m_Pu==0)
...
int ZMPVisualServoingQP::buildConstraintMatricesPLDPHerdt()
{
...
void ZMPVisualServoingQP::initFeet()
{
...
int ZMPVisualServoingQP::InitConstants()
{
  int r;
...
void ZMPVisualServoingQP::SetAlpha(const double &anAlpha)
{
  m_Alpha = anAlpha;
}

const double & ZMPVisualServoingQP::GetAlpha() const
{
  return m_Alpha;
}

void ZMPVisualServoingQP::SetBeta(const double &anAlpha)
{
  m_Beta = anAlpha;
}

const double & ZMPVisualServoingQP::GetBeta() const
{
  return m_Beta;
}
...
int ZMPVisualServoingQP::dumpProblem(MAL_VECTOR(& xk,double),
					 double Time)
{
  ofstream aof;
...
int ZMPVisualServoingQP::buildConstraintMatrices(double * &,
						     double * &DU,
						     double StartingTime,
...
//
//-----------new functions--------------
void ZMPVisualServoingQP::CallMethod(std::string & Method, std::istringstream &strm)
{
...
int ZMPVisualServoingQP::InitOnLine(deque<ZMPPosition> & FinalZMPPositions,
...
void ZMPVisualServoingQP::computeCholeskyOfQ(double * OptA)
{
...
void ZMPVisualServoingQP::computeObjective(deque<LinearConstraintInequalityFreeFeet_t> &
					       QueueOfLConstraintInequalitiesFreeFeet,
...
void ZMPVisualServoingQP::interpolateTrunkState(double time, int CurrentIndex,
...
void ZMPVisualServoingQP::interpolateFeetPositions(double time, int CurrentIndex,
						       deque<FootAbsolutePosition> &FinalLeftFootAbsolutePositions,
						       deque<FootAbsolutePosition> &FinalRightFootAbsolutePositions)
{
...
void ZMPVisualServoingQP::linearizeProjection()
{
  for(int i=0; i<m_Map.N; i++)
    {
      m_LinearizationTerms[i].UVector(0) = 1/m_Map.LandMarksInCamera[i](2,0);
      m_LinearizationTerms[i].UVector(1) = 0.0;
      m_LinearizationTerms[i].UVector(2) =  -m_Map.LandMarksInCamera[i](0,0)/
	(m_Map.LandMarksInCamera[i](2,0)*m_Map.LandMarksInCamera[i](2,0));
      m_LinearizationTerms[i].UScalar = m_Map.LandMarksInCamera[i](0,0)/m_Map.LandMarksInCamera[i](2,0);

      m_LinearizationTerms[i].VVector(0) = 0.0;
      m_LinearizationTerms[i].VVector(1) = 1/m_Map.LandMarksInCamera[i](2,0);
      m_LinearizationTerms[i].VVector(2) =  -m_Map.LandMarksInCamera[i](1,0)/
	(m_Map.LandMarksInCamera[i](2,0)*m_Map.LandMarksInCamera[i](2,0));
      m_LinearizationTerms[i].VScalar = m_Map.LandMarksInCamera[i](1,0)/m_Map.LandMarksInCamera[i](2,0);
...
void ZMPVisualServoingQP::GetZMPDiscretization(deque<ZMPPosition> & ,
						   deque<COMState> & ,
...
void ZMPVisualServoingQP::OnLineAddFoot(RelativeFootPosition & ,
					    deque<ZMPPosition> & ,
					    deque<COMState> & ,
					    deque<FootAbsolutePosition> &,
					    deque<FootAbsolutePosition> &,
...
int ZMPVisualServoingQP::OnLineFootChange(double ,
					      FootAbsolutePosition &,
					      deque<ZMPPosition> & ,
					      deque<COMState> & ,
					      deque<FootAbsolutePosition> &,
...
void ZMPVisualServoingQP::EndPhaseOfTheWalking(deque<ZMPPosition> &,
						   deque<COMState> &,
						   deque<FootAbsolutePosition> &,
						   deque<FootAbsolutePosition> &)
{
...
int ZMPVisualServoingQP::ReturnOptimalTimeToRegenerateAStep()
{
